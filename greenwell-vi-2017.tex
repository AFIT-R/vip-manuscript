...
\end{algorithm}
Algorithm~\eqref{alg:pdp} can be quite computationally intensive since it involves $k$ passes over the training records. Fortunately, the algorithm can be performed in parallel quite easily (more on this in Section~\ref{sec:computational}). It can also be easily extended to larger subsets of two or more features as well.
...
\begin{figure}[!htb]
  ...
  \caption{Variable importance plots for the neural network in Figure~\ref{fig:network}. \textit{Left}: partial dependence-based algorithm. \textit{Middle}: Garson's algorithm. \textit{Right}: Olden's algorithm. \label{fig:network-vip}}
\end{figure}
...
\begin{figure}[!htb]
  ...
  \caption{TBD. \label{fig:network-int}}
\end{figure}
...
\textbf{TODO:}
\begin{itemize}
  \item Talk about flexibility of the approach and using other more robust metrics such as the MAD/MADN estimator.
\end{itemize}
