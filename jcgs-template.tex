...
Constructing partial dependence functions using the brute force method described in Algorithm~\ref{alg:pdp} can be computationally expensive. Some solutions are discussed in \citet{pdp-greenwell-2017}. Fortunately, Algorithm~\ref{alg:pdp} is embarassingly parallel and computing single-predictor partial dependence functions for each predictor can be done rather quickly on a machine with a multi-core processor. For much larger data sets, it may be worthwhile to reduce the grid size by using specific quantiles for each predcitor. For example, the partial dependence function can be approximated by using the min and max of each predictor along with the deciles of the unique predictor values.
...
The same idea can be applied to finding interactions. Essentially a strong interaction effect of $x_1$ and $x_2$ on $\mathcal{Y}$ would suggest that $\imp\left(x_1, x_2\right)$ is roughly constant when either $x_1$ or $x_2$ is held constant while the other varies. 

More coming soon!

\begin{equation}
\label{eqn:interaction}
  \inter\left(x_i, x_j\right) = \left[\imp\left(x_i | x_j\right) + \imp\left(x_j | x_i\right)\right] / 2
\end{equation}


\subsection{Example: Friedman's regression problem}
